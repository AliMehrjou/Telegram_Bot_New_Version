...
async def handle_report_reason(call: CallbackQuery, state: FSMContext, db_session: AsyncSession) -> None:
    ...
    try:
        ...
        reported_id_from_cb = int(reported_id_str)
        match_id = int(match_id_str)
    except (ValueError, IndexError):
        await call.answer("خطا در پردازش اطلاعات.", show_alert=True)
        return

    # FIX HIGH-33: ensure the reported_id encoded in the callback matches the
    # one stored in FSM state — otherwise a stale button click could attribute
    # the report to the wrong match_history row.
    if reported_id_from_cb != reported_id:
        logger.warning(
            "safety_reason: callback reported_id=%s does not match FSM reported_id=%s (user %s)",
            reported_id_from_cb, reported_id, reporter_id,
        )
        await state.clear()
        await call.answer("⚠️ این دکمه متعلق به گزارش قدیمی است. لطفاً دوباره تلاش کنید.", show_alert=True)
        return
    ...
    worker.start_background_broadcast(user_ids=settings.parsed_admin_ids, text=admin_alert_text, delay_ms=40, on_blocked=mark_user_blocked)

    await state.clear()
    # FIX: wrap edit_text — the message may be older than 48h or already edited.
    ...
    await call.message.answer(
        "✅ گزارش شما با موفقیت ثبت شد. گفتگو پایان یافت و کاربر خاطی مسدود شد.", 
