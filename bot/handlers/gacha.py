...
def _generate_gacha_text(user: User, config: dict) -> str:
    ...
    level = max(1, user.level) # 🛡️ جلوگیری از لول صفر
    required_xp_for_level = level * 100
    ...
    return (
        texts.get("title", "") +
        texts.get("level_text", "").format(level=level) +
...
async def process_open_lootbox(call: CallbackQuery, db_session: AsyncSession):
    ...
    reward_type = str(selected_reward.get("type", "")).lower()
    amount = int(selected_reward.get("amount", 0))
