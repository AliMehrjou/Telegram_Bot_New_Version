...
from matching_bot_project.bot.states.states import GiftAddStates
...
from matching_bot_project.services.broadcast_worker import _get_rate_limiter
...
from matching_bot_project.services.payment_settings import (
    get_card_info,
    set_card_info,
    reset_card_info,
    is_overridden,
...
from matching_bot_project.database.models.models import BannerCampaign, BannerForward
from matching_bot_project.bot.filters.custom import IsAdminFilter
from aiogram.exceptions import TelegramBadRequest
from sqlalchemy import select
from matching_bot_project.database.models.models import BannerForward
...
# ایمپورت متغیرهای کش برای ریست کردن فوری آن‌ها
import matching_bot_project.services.vip_subscription as vip_sub

admin_router = Router(name="admin_vip_manager")

# ═══════════════════════════════════════════════════════════════════
# managing payment card
# ═══════════════════════════════════════════════════════════════════

def _cancel_set_card_keyboard() -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text="❌ انصراف", callback_data="cancel_set_card")]
    ])


def _confirm_set_card_keyboard() -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text="✅ تایید و ذخیره", callback_data="confirm_set_card")],
        [InlineKeyboardButton(text="❌ انصراف", callback_data="cancel_set_card")],
    ])


@router.message(Command("cardinfo"), IsAdminFilter())
async def cmd_card_info(message: Message):
    """نمایش شماره کارت و نام صاحب حساب فعلی که به کاربران نمایش داده می‌شود"""
    card_number, card_holder = await get_card_info()
    overridden = await is_overridden()
    source = "🔧 دستی (ذخیره‌شده توسط ادمین)" if overridden else "⚙️ پیش‌فرض (.env)"

    text = (
        "💳 <b>اطلاعات کارت فعال</b>\n"
        "━━━━━━━━━━━━━━━━━━━━\n"
    ...
    if overridden:
        text += "\nبرای بازگشت به مقدار پیش‌فرض: /resetcard"

    await message.answer(text, parse_mode="HTML")


@router.message(Command("setcard"), IsAdminFilter())
async def cmd_set_card_start(message: Message, state: FSMContext):
    """شروع فرآیند تغییر شماره کارت"""
    card_number, _ = await get_card_info()
    await state.set_state(AdminStates.waiting_for_card_number)
    await message.answer(
        "💳 <b>تغییر شماره کارت پرداخت</b>\n"
        "━━━━━━━━━━━━━━━━━━━━\n"
        f"شماره کارت فعلی: <code>{card_number}</code>\n\n"
        "لطفاً شماره کارت جدید را ارسال کنید (۱۶ رقم، با یا بدون خط تیره):",
        reply_markup=_cancel_set_card_keyboard(),
        parse_mode="HTML",
    )


@router.callback_query(F.data == "cancel_set_card")
async def cancel_set_card(call: CallbackQuery, state: FSMContext):
    await state.clear()
    await call.answer("عملیات لغو شد.")
    await call.message.edit_text("❌ تغییر شماره کارت لغو شد.")
...
async def process_new_card_number(message: Message, state: FSMContext):
    if message.text and message.text.startswith("/"):
        return await message.reply("⚠️ شما در حال تغییر شماره کارت هستید. برای انصراف از دکمه «❌ انصراف» استفاده کنید.")

    raw = (message.text or "").strip()
    digits_only = raw.replace("-", "").replace(" ", "")

    if not digits_only.isdigit() or len(digits_only) != 16:
        return await message.reply("⚠️ شماره کارت نامعتبر است. باید دقیقاً ۱۶ رقم باشد (خط تیره مجاز است).")

    
    await state.update_data(new_card_number=digits_only)
    await state.set_state(AdminStates.waiting_for_card_holder)
    await message.answer(
        "👤 حالا نام صاحب حساب را ارسال کنید:",
        reply_markup=_cancel_set_card_keyboard(),
    )
...
async def process_new_card_holder(message: Message, state: FSMContext):
    ...
    if not holder or len(holder) > 60:
        return await message.reply("⚠️ نام نامعتبر است (حداکثر ۶۰ کاراکتر و نمی‌تواند خالی باشد).")

    data = await state.get_data()
    card_number = data.get("new_card_number")
    if not card_number:
        await state.clear()
        return await message.answer("⚠️ نشست منقضی شده. لطفاً دوباره /setcard را بزنید.")

    await state.update_data(new_card_holder=holder)
    await state.set_state(AdminStates.confirming_card_change)

    await message.answer(
        "⚠️ <b>لطفاً اطلاعات را تایید کنید:</b>\n\n"
        f"شماره کارت: <code>{card_number}</code>\n"
        f"صاحب حساب: {holder}\n\n"
        "این اطلاعات از این پس به همه کاربران در بخش خرید سکه نمایش داده خواهد شد.",
        reply_markup=_confirm_set_card_keyboard(),
        parse_mode="HTML",
    )

@router.callback_query(AdminStates.confirming_card_change, F.data == "confirm_set_card")
async def confirm_set_card(call: CallbackQuery, state: FSMContext):
    data = await state.get_data()
    card_number = data.get("new_card_number")
    card_holder = data.get("new_card_holder")

    if not card_number or not card_holder:
        await state.clear()
        return await call.answer("⚠️ نشست منقضی شده بود.", show_alert=True)

    # FIX BUGS-3: Wrap set_card_info in try/except to gracefully handle ValueError
    try:
        await set_card_info(card_number, card_holder)
    except ValueError as e:
        await state.clear()
        await call.message.edit_text(f"❌ خطا در ذخیره‌سازی شماره کارت: {e}")
        return await call.answer("عملیات با خطا مواجه شد.", show_alert=True)

    await state.clear()
    await call.answer("✅ ذخیره شد.")
    await call.message.edit_text(
        "✅ <b>شماره کارت با موفقیت تغییر کرد.</b>\n\n"
        f"شماره کارت: <code>{card_number}</code>\n"
        f"صاحب حساب: {card_holder}",
        parse_mode="HTML",
    )
...
@router.message(Command("resetcard"), IsAdminFilter())
async def cmd_reset_card(message: Message):
    """بازگشت به شماره کارت پیش‌فرض تعریف‌شده در .env"""
    if not await is_overridden():
        return await message.answer("ℹ️ در حال حاضر شماره کارت پیش‌فرض (.env) فعال است.")

    await reset_card_info()
    card_number, card_holder = await get_card_info()
    await message.answer(
        "🔄 <b>بازگشت به مقدار پیش‌فرض انجام شد.</b>\n\n"
        f"شماره کارت: <code>{card_number}</code>\n"
        f"صاحب حساب: {card_holder}",
        parse_mode="HTML",
    )
...
async def cmd_list_pending_orders(message: Message, db_session: AsyncSession):
    """📋 نمایش لیست سفارش‌های در انتظار تأیید"""
    orders = await crud.get_pending_orders(db_session, limit=20)
    if not orders:
        return await message.answer("✅ هیچ سفارش در انتظاری وجود ندارد.")

    text = "📋 <b>سفارش‌های در انتظار تأیید:</b>\n"
    text += "━━━━━━━━━━━━━━━━━━━━\n\n"

    for order in orders:
        status_icon = "⏳" if order.status == "pending" else "✅" if order.status == "approved" else "❌"
        text += (
            f"{status_icon} <b>سفارش #{order.id}</b>\n"
            f"   👤 کاربر: <code>{order.user_tg_id}</code>\n"
            f"   💳 روش: {order.payment_method}\n"
            f"   📊 وضعیت: {order.status}\n"
            "   ──────────────────\n"
        )

    text += f"\n💡 برای جزئیات: <code>/order [id]</code>"
    await message.answer(text, parse_mode="HTML")


@router.message(Command("order"), IsAdminFilter())
async def cmd_get_order_details(message: Message, db_session: AsyncSession):
    """🔍 جستجوی سفارش با ID"""
    args = message.text.split()
    if len(args) != 2:
        return await message.answer(
            "❌ راهنما:\n<code>/order [order_id]</code>\nمثال: <code>/order 42</code>",
            parse_mode="HTML"
        )
    ...
    try:
        ...
        return await message.answer("❌ شناسه سفارش باید عدد باشد.")

    details = await crud.get_order_with_details(db_session, order_id)
    if not details:
        return await message.answer("❌ سفارشی با این شناسه یافت نشد.")
    ...
    text = (
        ...
        "━━━━━━━━━━━━━━━━━━━━\n"
        f"👤 <b>کاربر:</b> {user.first_name or 'نامشخص'} (<code>{user.tg_id}</code>)\n"
        f"📦 <b>بسته:</b> {package.coin_amount} سکه\n"
        f"💰 <b>مبلغ:</b> {package.price_toman:,} تومان\n"
        f"💳 <b>روش پرداخت:</b> {order.payment_method}\n"
        f"📊 <b>وضعیت:</b> {status_text}\n"
    )

    if hasattr(order, 'created_at') and order.created_at:
        text += f"📅 <b>زمان ثبت:</b> {order.created_at}\n"

    if order.resolved_at:
        text += f"✅ <b>زمان پردازش:</b> {order.resolved_at}\n"

    if order.gateway_authority:
        text += f"🔗 <b>Authority:</b> <code>{order.gateway_authority}</code>\n"

    # اگر سفارش pending است و فیش عکس دارد — عکس رو با دکمه‌ها بفرست
    if order.status == "pending":
        keyboard = InlineKeyboardMarkup(inline_keyboard=[[
            InlineKeyboardButton(text="✅ تأیید", callback_data=f"verify_receipt_{order.id}"),
            InlineKeyboardButton(text="❌ رد", callback_data=f"reject_receipt_{order.id}"),
        ]])

        if order.receipt_photo_file_id:
            await message.answer_photo(
                photo=order.receipt_photo_file_id,
                caption=text,
                parse_mode="HTML",
            ...
            text += "\n⚠️ این سفارش فیش عکس ندارد (پرداخت درگاه)."
...
async def cmd_addcoins(message: Message, db_session: AsyncSession):
    ...
    # FIX HIGH-34: /addcoins must reject non-positive amounts (otherwise it silently deducts).
    if amount <= 0:
        return await message.answer("⚠️ Amount must be positive. Use /removecoins to deduct.")
...
async def cmd_addcoinsall(message: Message, db_session: AsyncSession):
    ...
    if amount <= 0:
        return await message.answer("⚠️ Amount must be positive.")

    # FIX CRIT-08: don't stream + ORM-update on the same session — server-side cursor
    # becomes invalid and ORM objects accumulate (OOM risk on large user bases).
    # Instead: read all tg_ids once, then process in batches with fresh sessions.
    from matching_bot_project.database.session import async_session_factory
    # کوئری قبلی: select(User.tg_id)
    result = await db_session.execute(select(User.tg_id).where(User.re_engage_blocked == False))
    ...
    for i in range(0, len(user_ids), BATCH):
        ...
        async with async_session_factory() as batch_session:
            for uid in batch:
                u = await get_user_by_tg_id(batch_session, uid)
                if u:
                    await process_coin_transaction(
                        batch_session, u, amount, "Global admin reward", ignore_multiplier=True,
                    )
            await batch_session.commit()
        processed += len(batch)
        if processed % 1000 == 0:
            logger.info("addcoinsall: processed %s/%s users", processed, len(user_ids))
...
async def cmd_addcoinsvip(message: Message, db_session: AsyncSession):
    ...
    # FIX CRIT-08: batched approach to avoid OOM and cursor invalidation.
    from matching_bot_project.database.session import async_session_factory
    ...
    result = await db_session.execute(
        select(User.tg_id).where(
            and_(
                User.is_vip == True,
                User.re_engage_blocked == False,  # این خط اضافه شد
                or_(User.vip_expires_at == None, User.vip_expires_at > now_utc)
            )
        )
    )
    user_ids = [int(r) for r in result.scalars().all()]
    if not user_ids:
        return await message.answer("No VIP users found.")
    ...
    for i in range(0, len(user_ids), BATCH):
        ...
        async with async_session_factory() as batch_session:
            for uid in batch:
                ...
                if u:
                    await process_coin_transaction(
                        batch_session, u, amount, "VIP admin reward", ignore_multiplier=True,
                    )
            await batch_session.commit()
...
async def cmd_banuser(message: Message, db_session: AsyncSession):
    ...
    if tg_id == message.from_user.id:
        return await message.answer("⚠️ شما نمی‌توانید خودتان را مسدود کنید!")
    ...
    if active_match:
        ...
        try:
            ...
            await partner_ctx.clear() # 🟢 فیکس زامبی شدن پارتنر
...
async def cmd_setvip(message: Message, db_session: AsyncSession):
    ...
    # FIX M-28: reject non-positive days — otherwise vip_expires_at lands in the past.
    if days <= 0:
        return await message.answer("⚠️ Days must be positive.")
...
@admin_router.message(Command("setvipprice"))
async def set_vip_price_command(message: Message):
    # بررسی ادمین بودن
    if message.from_user.id not in settings.parsed_admin_ids:
        return
        
    args = message.text.split()
    if len(args) != 3:
        return await message.answer(
            "⚠️ <b>فرمت صحیح دستور:</b>\n"
            "<code>/setvipprice [1w/2w/1m] [قیمت_به_تومان]</code>\n\n"
            "مثال: <code>/setvipprice 1m 150000</code>",
            parse_mode="HTML"
        )
        
    plan_code, new_price_str = args[1], args[2]
    try:
        new_price = int(new_price_str)
    except ValueError:
        return await message.answer("⚠️ قیمت باید فقط شامل اعداد باشد.")
    ...
    try:
        ...
        return await message.answer("❌ فایل vip_plans.json یافت نشد.")
        
    if plan_code not in plans:
        return await message.answer("❌ کد پلن اشتباه است. مقادیر مجاز: 1w, 2w, 1m")
        
    # تغییر قیمت و ذخیره در فایل
    plans[plan_code]["price_toman"] = new_price
    
    with open(json_path, "w", encoding="utf-8") as f:
        json.dump(plans, f, ensure_ascii=False, indent=2)
        
    # 🔥 ریست کردن کش برای اعمال فوری در کل ربات
    vip_sub._VIP_PLANS_CACHE = None
    vip_sub._VIP_PLANS_LOADED_AT = None
    
    await message.answer(f"✅ <b>موفقیت‌آمیز!</b>\nقیمت پلن <b>{plan_code}</b> در سیستم به <b>{new_price:,} تومان</b> تغییر یافت.", parse_mode="HTML")
...
async def cq_stats(call: CallbackQuery, db_read_session: AsyncSession):
    action = call.data

    if action == "stats_today_reg":
        today = datetime.now(timezone.utc).replace(hour=0, minute=0, second=0, microsecond=0)
        count = await db_read_session.scalar(select(func.count(User.id)).where(User.created_at >= today))
...
async def admin_quick_ban(call: CallbackQuery, db_session: AsyncSession):
    ...
    user.is_banned = True
    await redis_client.set(f"user:banned:{target_tg_id}", "1")
...
async def admin_quick_unban(call: CallbackQuery, db_session: AsyncSession):
    ...
    await redis_client.delete(f"user:banned:{target_tg_id}")
    await db_session.commit()
...
async def cmd_broadcast(message: Message, state: FSMContext, db_session: AsyncSession):
    """آغاز فرآیند ارسال پیام همگانی توسط ادمین"""
    
    # 🛡️ رفع باگ تداخل FSM: بررسی وضعیت دیت/چت ادمین
    from matching_bot_project.database.queries import crud
    active_match = await crud.get_active_match(db_session, message.from_user.id)
    if active_match:
        return await message.answer(
            "⚠️ <b>اخطار تداخل:</b> شما در حال حاضر در یک چت/دیت فعال هستید!\n"
            "ورود به بخش ارسال پیام همگانی باعث تغییر وضعیت شما و خروج از دیت می‌شود. لطفاً ابتدا دیت خود را متوقف کنید.",
            parse_mode="HTML"
        )
...
async def process_broadcast_message(message: Message, state: FSMContext, db_session: AsyncSession):
    """دریافت محتوا از ادمین و پرسش برای پین کردن پیام"""
    ...
    # ذخیره پیام در حافظه موقت (FSM)
    await state.update_data(
        broadcast_from_chat_id=message.chat.id,
        broadcast_message_id=message.message_id
    )
    
    # نمایش گزینه‌های پین کردن به ادمین
    kb = InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text="📢 ارسال عادی", callback_data="broadcast_confirm_normal")],
        [InlineKeyboardButton(text="📌 ارسال و پین در پی‌وی کاربران", callback_data="broadcast_confirm_pin")],
        [InlineKeyboardButton(text="❌ انصراف", callback_data="cancel_broadcast")]
    ])
    
    await message.answer(
        "✅ پیام شما دریافت شد. لطفاً نحوه ارسال را انتخاب کنید:",
        reply_markup=kb,
        parse_mode="HTML"
    )

# 💡 هندلر جدید برای پردازش انتخاب ادمین (ارسال عادی یا پین)
@router.callback_query(F.data.startswith("broadcast_confirm_"))
async def confirm_broadcast_action(call: CallbackQuery, state: FSMContext, db_session: AsyncSession):
    data = await state.get_data()
    ...
    if not user_ids:
        await state.clear()
        return await call.message.edit_text("هیچ کاربری در دیتابیس یافت نشد.")

    # ارسال به ورکر با پارامتر پین
    worker = BroadcastWorker(bot=bot)
    worker.start_background_broadcast(
        ...
        on_blocked=mark_user_blocked
    )

    await state.clear()
    
    pin_status_text = "و <b>پین شدن</b> " if pin_in_chats else ""
    await call.message.edit_text(
        f"✅ <b>عملیات موفق</b>\n\n"
        f"ارسال {pin_status_text}پیام همگانی در پس‌زمینه آغاز شد.\n"
...
async def event_confirm(call: CallbackQuery, state: FSMContext, db_session: AsyncSession) -> None:
    ...
    # FIX M-22: use .get() with defaults so an evicted FSM key doesn't crash the handler.
    duration_minutes = data.get("event_duration_minutes")
    multiplier = data.get("event_multiplier")
    event_name = data.get("event_name", "رویداد")
    event_description = data.get("event_description", "")

    if duration_minutes is None or multiplier is None:
        logger.warning("event_confirm: FSM data evicted for user %s", call.from_user.id)
        try:
            await call.message.answer("⚠️ نشست منقضی شده. لطفاً رویداد را دوباره بسازید.")
        except Exception:
            pass
        return
    ...
    # FIX M-22: wrap Redis in try/except so a Redis outage surfaces a clean error.
    try:
        ...
        logger.error("event_confirm: Redis failure: %s", exc)
        try:
            await call.message.answer("⚠️ خطای ارتباط با سرور. رویداد ذخیره نشد.")
        except Exception:
            pass
        return
    ...
    worker = BroadcastWorker(bot=bot)
    worker.start_background_broadcast(user_ids=user_ids, text=notification_text, delay_ms=40, on_blocked=mark_user_blocked)
...
def _personalize(template: str, user: User) -> str:
    ...
    FIX HIGH-30: escape user-controlled values (first_name, city) so a malicious
    user can't inject HTML tags into broadcast messages sent with parse_mode="HTML".
    Numeric values are safe by construction.
    """
    import html as _html
    return (
        template
        .replace("{name}",  _html.escape(user.first_name or "دوست عزیز"))
        .replace("{city}",  _html.escape(user.city or "ایران"))
...
async def _background_pbroadcast(users_raw_data: list, template: str):
    ...
    # ضرب کردن سرعت مجاز در تعداد ربات‌های فعال (Shardها)
    total_rate = float(settings.TG_OUTBOUND_RATE_PER_BOT) * shard_manager.num_shards
    rate_limiter = _get_rate_limiter(total_rate)
    ...
    for tg_id, fname, city, coins, age in users_raw_data:
        ...
        try:
            # 🟢 دریافت ربات اختصاصی کاربر از شارد منیجر
            user_bot = await shard_manager.get_bot_for_user_async(tg_id)
            # 🟢 ارسال پیام با ربات اختصاصی
            await user_bot.send_message(chat_id=tg_id, text=text, parse_mode="HTML")
            sent += 1
        except Exception as e:
            logger.debug(f"PBroadcast failed for {tg_id}: {e}")
            failed += 1
...
async def build_daily_report(db_read_session: AsyncSession) -> str:
...
async def _daily_report_loop(session_factory) -> None:
    ...
    در api/main.py با asyncio.create_task صدا بزن.
...
async def cmd_addsponsor(message: Message):
    ...
    try:
        current_sponsors_count = await redis_client.hlen("bot:sponsors")
        if current_sponsors_count >= 5:
            return await message.answer(
                "⚠️ <b>خطا:</b> شما به حداکثر تعداد کانال‌های اسپانسر (۵ عدد) رسیده‌اید!\n"
                "برای افزودن کانال جدید، ابتدا یکی از کانال‌های قبلی را با <code>/removesponsor</code> حذف کنید.",
                parse_mode="HTML"
            )
    except Exception as e:
        logger.error(f"Failed to check sponsors count in Redis: {e}")
        return await message.answer(
            "⚠️ <b>خطا در ارتباط با سرور:</b>\nلطفاً چند لحظه دیگر تلاش کنید.",
            parse_mode="HTML"
        )
    ...
    # 🛡️ رفع بحران FloodWait: حذفِ افزایش سراسری ورژن کش (Hard Invalidation)
    # به جای این کار، سیستم از Soft Invalidation استفاده می‌کند (کش هر کاربر پس از ۵ دقیقه به طور طبیعی منقضی می‌شود)
    # await redis_client.incr("bot:sponsors_version")

    await message.answer(
        f"✅ کانال <code>{channel_id}</code> با موفقیت تایید و به لیست اسپانسرها اضافه شد.\n"
        f"⚡️ <b>توجه:</b> برای جلوگیری از لیمیت شدن ربات تلگرام، این تغییر به صورت تدریجی (طی ۵ دقیقه آینده) برای کاربران اعمال خواهد شد.",
        parse_mode="HTML"
    )
...
async def cmd_removesponsor(message: Message):
    ...
    if deleted:
        # 🛡️ رفع بحران FloodWait: حذفِ افزایش سراسری ورژن کش
        # await redis_client.incr("bot:sponsors_version")
        await message.answer(
            f"🗑 کانال <code>{channel_id}</code> از لیست اسپانسرها حذف شد.\n"
            f"⚡️ بررسی عضویت این کانال برای کاربران متوقف شد.",
            parse_mode="HTML"
        )
    else:
...
# ═══════════════════════════════════════════════════════════════════════════
# FIX PHASE5-CRIT-05: Admin report & banner review handlers (in-bot)
# ═══════════════════════════════════════════════════════════════════════════
# Previously, the keyboards get_admin_report_review_keyboard and
# get_admin_banner_review_keyboard emitted `admin_report_approve_{id}`,
# `admin_report_reject_{id}`, `admin_banner_approve_{id}`,
# `admin_banner_reject_{id}` — but NO handler in bot/handlers/admin.py
# registered these prefixes. The 3-strike warning system could not be
# operated from inside the bot. Admins had to use the HTTP API
# (api/routes/admin.py) instead.
# These handlers wire the in-bot review buttons to the same
# warning_engine.approve_report / reject_report and
# free_coin_banner_service.approve_forward / reject_forward methods
# used by the HTTP API.


@router.callback_query(F.data.startswith("admin_report_approve_"))
async def bot_admin_approve_report(call: CallbackQuery, db_session: AsyncSession):
    """Admin tapped 'approve report' in the bot → reward reporter, warn reported user."""
    if call.from_user.id not in settings.parsed_admin_ids:
        return await call.answer("دسترسی غیرمجاز.", show_alert=True)
    try:
        report_id = int(call.data.replace("admin_report_approve_", ""))
    except ValueError:
        return await call.answer("شناسه گزارش نامعتبر.", show_alert=True)

    from matching_bot_project.bot.core.loader import warning_engine
    result = await warning_engine.approve_report(db_session, report_id, call.from_user.id)
    
    if "error" in result:
        msg_text = f"❌ خطا: {result['error']}"
    else:
        wr = result.get("warning_result", {})
        msg_text = (
            f"✅ <b>گزارش #{report_id} تأیید شد.</b>\n\n"
            f"👤 متخلف: <code>{result.get('reported_tg_id')}</code>\n"
            f"⚠️ اخطارهای فعلی: {wr.get('warning_count', '?')} / {wr.get('max_warnings', '?')}\n"
            f"🚫 مسدود شد: {'بله' if wr.get('is_banned') else 'خیر'}\n\n"
            f"🎁 پاداش گزارش‌دهنده (<code>{result.get('reporter_tg_id')}</code>): "
            f"{result.get('reward_coins', 0)} سکه"
        )
    ...
    try:
        if call.message.photo or call.message.video or call.message.document:
            await call.message.edit_caption(caption=msg_text, parse_mode="HTML", reply_markup=None)
        else:
            await call.message.edit_text(text=msg_text, parse_mode="HTML", reply_markup=None)
...
async def bot_admin_reject_report(call: CallbackQuery, db_session: AsyncSession):
    """Admin tapped 'reject report' in the bot → warn the reporter (false report)."""
    if call.from_user.id not in settings.parsed_admin_ids:
        return await call.answer("دسترسی غیرمجاز.", show_alert=True)
    try:
        report_id = int(call.data.replace("admin_report_reject_", ""))
    except ValueError:
        return await call.answer("شناسه گزارش نامعتبر.", show_alert=True)

    from matching_bot_project.bot.core.loader import warning_engine
    result = await warning_engine.reject_report(db_session, report_id, call.from_user.id)
    ...
    if "error" in result:
        ...
        wr = result.get("warning_result", {})
        msg_text = (
            f"❌ <b>گزارش #{report_id} رد شد (گزارش کاذب).</b>\n\n"
            f"👤 گزارش‌دهنده: <code>{result.get('reporter_tg_id')}</code>\n"
            f"⚠️ اخطارهای فعلی: {wr.get('warning_count', '?')} / {wr.get('max_warnings', '?')}\n"
            f"🚫 مسدود شد: {'بله' if wr.get('is_banned') else 'خیر'}"
        )
...
async def cancel_pbroadcast_handler(call: CallbackQuery, state: FSMContext) -> None:
    """لغو فرآیند ارسال پیام همگانی شخصی‌سازی‌شده (pbroadcast)"""
    await call.answer("عملیات لغو شد.")
    await state.clear()
    try:
        await call.message.edit_text("❌ فرآیند ارسال پیام شخصی‌سازی‌شده لغو شد.")
    except Exception:
        pass


@router.callback_query(F.data.startswith("admin_banner_approve_"))
async def bot_admin_approve_banner(call: CallbackQuery, db_session: AsyncSession):
    """Admin tapped 'approve banner forward' → credit user with reward_coins and notify them."""
    if call.from_user.id not in settings.parsed_admin_ids:
        return await call.answer("دسترسی غیرمجاز.", show_alert=True)
    try:
        forward_id = int(call.data.replace("admin_banner_approve_", ""))
    except ValueError:
    ...
    result = await free_coin_banner_service.approve_forward(db_session, forward_id, call.from_user.id)
    
    if "error" in result:
        msg_text = f"❌ خطا: {result['error']}"
    else:
        user_tg_id = result.get('user_tg_id')
        reward = result.get('reward_coins', 0)
        
        msg_text = (
            f"✅ <b>بنر #{forward_id} تأیید شد.</b>\n\n"
            f"👤 کاربر: <code>{user_tg_id}</code>\n"
            f"🎁 پاداش: {reward} سکه"
        )
        
        # 🟢 ارسال نوتیفیکیشن به کاربر بابت تایید و دریافت سکه
        if user_tg_id:
            try:
                await bot.send_message(
                    chat_id=user_tg_id,
                    text=f"🎁 <b>تبریک!</b>\n\nاسکرین‌شات بنر شما توسط مدیریت تأیید شد و <b>{reward} سکه</b> به حساب شما واریز گردید. از حمایت شما سپاسگزاریم!",
                    parse_mode="HTML"
                )
            except Exception as e:
                logger.warning(f"Could not notify user {user_tg_id} about banner approval: {e}")
...
@router.callback_query(F.data.startswith("admin_banner_reject_"))
async def bot_admin_reject_banner(call: CallbackQuery, db_session: AsyncSession):
    """Admin tapped 'reject banner forward' → notify the user."""
    if call.from_user.id not in settings.parsed_admin_ids:
        return await call.answer("دسترسی غیرمجاز.", show_alert=True)
    try:
        forward_id = int(call.data.replace("admin_banner_reject_", ""))
    except ValueError:
        return await call.answer("شناسه نامعتبر.", show_alert=True)

    from matching_bot_project.bot.core.loader import free_coin_banner_service, bot
    
    # 🟢 استخراج آیدی کاربر قبل از رد کردن بنر (چون سرویس reject آیدی رو برنمی‌گردونه)
    fw_req = await db_session.execute(select(BannerForward).where(BannerForward.id == forward_id))
    fw = fw_req.scalar_one_or_none()
    user_tg_id = fw.user_tg_id if fw else None

    result = await free_coin_banner_service.reject_forward(
        db_session, forward_id, call.from_user.id, "رد شده توسط ادمین"
    )
    
    if "error" in result:
        msg_text = f"❌ خطا: {result['error']}"
    else:
        msg_text = f"❌ <b>بنر #{forward_id} رد شد.</b>"
        
        # 🟢 ارسال نوتیفیکیشن به کاربر بابت رد شدن مدرک
        if user_tg_id:
            try:
                await bot.send_message(
                    chat_id=user_tg_id,
                    text="❌ <b>متأسفانه مدرک شما برای دریافت سکه رایگان تأیید نشد.</b>\n\nلطفاً دقت کنید که تصویر واضح باشد و بنر را به درستی در یک کانال/گروه ارسال کرده باشید. می‌توانید مجدداً از منوی سکه تلاش کنید.",
                    parse_mode="HTML"
                )
            except Exception as e:
                logger.warning(f"Could not notify user {user_tg_id} about banner rejection: {e}")
...
@router.callback_query(F.data == "vip_show_plans")
async def bot_vip_show_plans(call: CallbackQuery, db_session: AsyncSession):
    """Non-VIP user tapped 'upgrade to VIP' → show VIP subscription plans."""
    # Delegate to the existing VIP panel handler by re-routing.
    # We can't call the handler directly (it expects a specific signature),
    # so we just show the VIP plans menu.
    ...
    if not user:
        return await call.answer("ابتدا ثبت‌نام کنید.", show_alert=True)

    plans = load_vip_plans()
    if not plans:
        return await call.answer("در حال حاضر پلن VIP موجود نیست.", show_alert=True)

    text = "💎 <b>اشتراک VIP</b>\n\n"
    for code, plan in plans.items():
        text += f"• <b>{plan.get('label', code)}</b>: {plan.get('duration_days', '?')} روز — {plan.get('price_toman', '?')} تومان\n"
    text += "\nبرای خرید، روی یکی از پلن‌های زیر بزنید:"

    try:
        await call.message.edit_text(text, parse_mode="HTML", reply_markup=get_vip_subscription_plans_keyboard(plans))
    except Exception:
        await call.message.answer(text, parse_mode="HTML", reply_markup=get_vip_subscription_plans_keyboard(plans))
    await call.answer()


def _cancel_gift_add_keyboard() -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text="❌ انصراف", callback_data="cancel_add_gift")]
    ])

@router.message(Command("addgift"), IsAdminFilter())
async def cmd_add_gift(message: Message, state: FSMContext):
    """شروع فرآیند افزودن گیفت جدید"""
    await state.set_state(GiftAddStates.waiting_for_code)
    await message.answer(
        "🎁 <b>افزودن گیفت جدید</b>\n"
        "━━━━━━━━━━━━━━━━━━━━\n"
        "مرحله ۱/۵ — لطفاً یک <b>کد انگلیسی (شناسه)</b> برای گیفت وارد کنید (بدون فاصله):\n"
        "<i>مثال: ROSE یا COFFEE یا WATCH</i>",
        parse_mode="HTML",
        reply_markup=_cancel_gift_add_keyboard()
    )

@router.callback_query(F.data == "cancel_add_gift")
async def cancel_add_gift(call: CallbackQuery, state: FSMContext):
    await state.clear()
    await call.answer("عملیات لغو شد.")
    try:
        await call.message.edit_text("❌ فرآیند افزودن گیفت لغو شد.")
    except Exception:
        pass

@router.message(GiftAddStates.waiting_for_code, IsAdminFilter())
async def process_gift_code(message: Message, state: FSMContext, db_session: AsyncSession):
    code = (message.text or "").strip().upper()
    if not code or " " in code:
        return await message.answer("⚠️ کد نباید خالی باشد و نباید فاصله داشته باشد. دوباره وارد کنید:")
    
    # بررسی تکراری نبودن کد
    exists = await crud.check_gift_code_exists(db_session, code)
    if exists:
        return await message.answer("⚠️ این کد قبلاً برای یک گیفت دیگر ثبت شده است. یک کد جدید وارد کنید:")

    await state.update_data(gift_code=code)
    await state.set_state(GiftAddStates.waiting_for_name)
    await message.answer(
        "مرحله ۲/۵ — <b>نام نمایشی (فارسی)</b> گیفت را وارد کنید:\n"
        "<i>مثال: گل رز، قهوه، ساعت مچی</i>",
        parse_mode="HTML",
        reply_markup=_cancel_gift_add_keyboard()
    )

@router.message(GiftAddStates.waiting_for_name, IsAdminFilter())
async def process_gift_name(message: Message, state: FSMContext):
    name = (message.text or "").strip()
    if not name:
        return await message.answer("⚠️ نام گیفت نمی‌تواند خالی باشد. دوباره وارد کنید:")
        
    await state.update_data(gift_name=name)
    await state.set_state(GiftAddStates.waiting_for_emoji)
    await message.answer(
        "مرحله ۳/۵ — یک <b>ایموجی</b> مناسب برای این گیفت ارسال کنید:\n"
        "<i>مثال: 🌹 یا ☕️</i>",
        parse_mode="HTML",
        reply_markup=_cancel_gift_add_keyboard()
    )

@router.message(GiftAddStates.waiting_for_emoji, IsAdminFilter())
async def process_gift_emoji(message: Message, state: FSMContext):
    emoji = (message.text or "").strip()
    if not emoji:
        return await message.answer("⚠️ لطفاً یک ایموجی ارسال کنید:")
        
    await state.update_data(gift_emoji=emoji)
    await state.set_state(GiftAddStates.waiting_for_price)
    await message.answer(
        "مرحله ۴/۵ — <b>قیمت گیفت</b> (به سکه) را وارد کنید:\n"
        "<i>مثال: 25</i>",
        parse_mode="HTML",
        reply_markup=_cancel_gift_add_keyboard()
    )

@router.message(GiftAddStates.waiting_for_price, IsAdminFilter())
async def process_gift_price(message: Message, state: FSMContext):
    try:
        ...
        return await message.answer("⚠️ قیمت باید یک عدد صحیح و بزرگتر از صفر باشد. دوباره وارد کنید:")
        
    await state.update_data(gift_price=price)
    await state.set_state(GiftAddStates.waiting_for_description)
    await message.answer(
        "مرحله ۵/۵ — <b>توضیح کوتاه</b> برای گیفت بنویسید:\n"
        "<i>مثال: یک کادوی خاص برای شروع یک آشنایی فوق‌العاده</i>",
        parse_mode="HTML",
        reply_markup=_cancel_gift_add_keyboard()
    )

@router.message(GiftAddStates.waiting_for_description, IsAdminFilter())
async def process_gift_description(message: Message, state: FSMContext):
    desc = (message.text or "").strip()
    await state.update_data(gift_desc=desc)
    
    data = await state.get_data()
    await state.set_state(GiftAddStates.confirming)
    
    confirm_kb = InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text="✅ تایید و ساخت گیفت", callback_data="confirm_add_gift")],
        [InlineKeyboardButton(text="❌ انصراف", callback_data="cancel_add_gift")]
    ])
    
    await message.answer(
        "📋 <b>تایید نهایی مشخصات گیفت:</b>\n\n"
        f"🏷 شناسه: <code>{data['gift_code']}</code>\n"
        f"📌 نام: <b>{data['gift_name']}</b>\n"
        f"🎨 ایموجی: {data['gift_emoji']}\n"
        f"💰 قیمت: <b>{data['gift_price']}</b> سکه\n"
        f"📝 توضیح: <i>{data['gift_desc']}</i>\n\n"
        "آیا این گیفت ذخیره شود؟",
        parse_mode="HTML",
        reply_markup=confirm_kb
    )

@router.callback_query(GiftAddStates.confirming, F.data == "confirm_add_gift")
async def confirm_add_gift(call: CallbackQuery, state: FSMContext, db_session: AsyncSession):
    data = await state.get_data()
    
    if not data:
        return await call.answer("⚠️ نشست منقضی شده است. لطفا دوباره تلاش کنید.", show_alert=True)
        
    try:
        await crud.add_gift_type(
            session=db_session,
            code=data['gift_code'],
            display_name=data['gift_name'],
            emoji=data['gift_emoji'],
            price_coins=data['gift_price'],
            description=data['gift_desc']
        )
        await db_session.commit()
        
        await call.message.edit_text(
            f"✅ گیفت <b>{data['gift_emoji']} {data['gift_name']}</b> با موفقیت به سیستم اضافه شد و هم‌اکنون برای کاربران قابل خریداری است.",
            parse_mode="HTML"
        )
    except Exception as e:
        await db_session.rollback()
        await call.message.edit_text(f"❌ خطایی در ذخیره گیفت رخ داد: {e}")
        
    await state.clear()
    await call.answer()

# ══════════════════════════════════════════════════════════════
# سیستم مدیریت بنرهای سکه رایگان (Free Coin Banners)
# ══════════════════════════════════════════════════════════════
...
def _cancel_banner_keyboard() -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text="❌ انصراف", callback_data="banner_cancel")]
    ])

@router.message(Command("newbanner"), IsAdminFilter())
async def cmd_newbanner(message: Message, state: FSMContext):
    """شروع فرآیند ساخت کمپین بنری جدید"""
    await state.set_state(BannerAddStates.waiting_for_photo)
    await message.answer(
        "🖼 <b>ساخت کمپین بنر جدید (سکه رایگان)</b>\n"
        "━━━━━━━━━━━━━━━━━━━━\n"
        "مرحله ۱/۳ — لطفاً عکس بنر را ارسال کنید:\n"
        "<i>(فقط یک تصویر ارسال کنید)</i>",
        parse_mode="HTML",
        reply_markup=_cancel_banner_keyboard()
    )

@router.callback_query(F.data == "banner_cancel")
async def cancel_banner_creation(call: CallbackQuery, state: FSMContext):
    await state.clear()
    await call.message.edit_text("❌ عملیات ساخت بنر لغو شد.")
    await call.answer()

@router.message(BannerAddStates.waiting_for_photo, IsAdminFilter(), F.photo)
async def process_banner_photo(message: Message, state: FSMContext):
    photo_file_id = message.photo[-1].file_id
    await state.update_data(banner_photo=photo_file_id)
    await state.set_state(BannerAddStates.waiting_for_caption)
    await message.answer(
        "مرحله ۲/۳ — لطفاً <b>متن (کپشن)</b> بنر را بفرستید:\n"
        "<i>(شامل توضیحات و لینک‌های مورد نیاز)</i>",
        parse_mode="HTML",
        reply_markup=_cancel_banner_keyboard()
    )

@router.message(BannerAddStates.waiting_for_photo, IsAdminFilter())
async def process_banner_photo_invalid(message: Message):
    if message.text and message.text.startswith("/"):
        return
    await message.answer("⚠️ لطفاً فقط یک تصویر (عکس) ارسال کنید.", reply_markup=_cancel_banner_keyboard())

@router.message(BannerAddStates.waiting_for_caption, IsAdminFilter())
async def process_banner_caption(message: Message, state: FSMContext):
    caption = message.html_text or message.text or ""
    if not caption.strip():
        return await message.answer("⚠️ متن نمی‌تواند خالی باشد. دوباره بفرستید:")
        
    await state.update_data(banner_caption=caption)
    await state.set_state(BannerAddStates.waiting_for_reward)
    await message.answer(
        "مرحله ۳/۳ — <b>مقدار پاداش (سکه)</b> برای این بنر چقدر باشد؟\n"
        "<i>(یک عدد صحیح بزرگتر از ۰ وارد کنید. پیش‌فرض: ۲)</i>",
        parse_mode="HTML",
        reply_markup=_cancel_banner_keyboard()
    )

@router.message(BannerAddStates.waiting_for_reward, IsAdminFilter())
async def process_banner_reward(message: Message, state: FSMContext, db_session: AsyncSession):
    text = (message.text or "").strip()
    try:
        reward = int(text)
        if reward <= 0:
    ...
    await state.update_data(banner_reward=reward)
    data = await state.get_data()
    
    # بررسی اینکه آیا کمپین فعالی از قبل وجود دارد یا خیر
    active_campaign = await free_coin_banner_service.get_active_campaign(db_session)
    await state.set_state(BannerAddStates.confirming)
    
    keyboard = []
    if active_campaign:
        keyboard.append([InlineKeyboardButton(text="🔄 غیرفعال کردن قبلی و ساخت جدید", callback_data="banner_confirm_replace")])
        keyboard.append([InlineKeyboardButton(text="✅ ساخت (هر دو فعال بمانند)", callback_data="banner_confirm_keep")])
        warn_text = f"\n\n⚠️ <b>توجه:</b> یک کمپین فعال (شناسه <code>{active_campaign.id}</code>) از قبل وجود دارد!"
    else:
        keyboard.append([InlineKeyboardButton(text="✅ تایید و فعال‌سازی", callback_data="banner_confirm_keep")])
        warn_text = ""
        
    keyboard.append([InlineKeyboardButton(text="❌ انصراف", callback_data="banner_cancel")])
    
    await message.answer_photo(
        photo=data['banner_photo'],
        caption=(
            f"📋 <b>تایید نهایی کمپین:</b>\n\n"
            f"متن بنر:\n{data['banner_caption']}\n\n"
            f"💰 پاداش: <b>{reward}</b> سکه{warn_text}\n\n"
            f"آیا این کمپین ذخیره و فعال شود؟"
        ),
        parse_mode="HTML",
        reply_markup=InlineKeyboardMarkup(inline_keyboard=keyboard)
    )

@router.callback_query(BannerAddStates.confirming, F.data.in_({"banner_confirm_keep", "banner_confirm_replace"}))
async def confirm_banner_creation(call: CallbackQuery, state: FSMContext, db_session: AsyncSession):
    data = await state.get_data()
    action = call.data
    
    if not data:
        return await call.answer("⚠️ نشست منقضی شده است.", show_alert=True)
        
    # اگر ادمین تصمیم گرفت کمپین قبلی را غیرفعال کند
    if action == "banner_confirm_replace":
        active_campaign = await free_coin_banner_service.get_active_campaign(db_session)
        if active_campaign:
            active_campaign.is_active = False
            await db_session.commit()
            
    campaign = await free_coin_banner_service.create_campaign(
        session=db_session,
        banner_photo_file_id=data['banner_photo'],
        caption_text=data['banner_caption'],
        reward_coins=data['banner_reward']
    )
    
    await state.clear()
    
    await call.message.edit_caption(
        caption=f"✅ <b>کمپین بنری با موفقیت ایجاد و فعال شد!</b>\n\nشناسه کمپین: <code>{campaign.id}</code>\nپاداش: {campaign.reward_coins} سکه",
        parse_mode="HTML",
        reply_markup=None
    )
    await call.answer()

@router.message(Command("banners"), IsAdminFilter())
async def cmd_banners(message: Message, db_session: AsyncSession):
    """لیست کمپین‌های بنری و آمار فورواردهای آن‌ها"""
    
    # کوئری JOIN برای استخراج تعداد فورواردها همراه با دیتای بنر
    stmt = (
        select(BannerCampaign, func.count(BannerForward.id))
        .outerjoin(BannerForward, BannerCampaign.id == BannerForward.campaign_id)
        .group_by(BannerCampaign.id)
        .order_by(BannerCampaign.id.desc())
        .limit(15)  # نمایش 15 کمپین آخر برای جلوگیری از پیام طولانی
    )
    result = await db_session.execute(stmt)
    records = result.all()
    
    if not records:
        return await message.answer("📭 هیچ کمپین بنری در سیستم ثبت نشده است.")
        
    text = "📢 <b>لیست کمپین‌های بنری (۱۵ مورد اخیر):</b>\n━━━━━━━━━━━━━━━━━━━━\n\n"
    
    for campaign, fwd_count in records:
        status = "🟢 فعال" if campaign.is_active else "🔴 غیرفعال"
        created_at = campaign.created_at.strftime('%Y-%m-%d %H:%M') if campaign.created_at else 'نامشخص'
        
        text += (
            f"🔹 <b>شناسه:</b> <code>{campaign.id}</code> | {status}\n"
            f"💰 <b>پاداش:</b> {campaign.reward_coins} سکه\n"
            f"📊 <b>تعداد فوروارد ثبت‌شده:</b> {fwd_count}\n"
...
@router.message(Command("sync_states"), IsAdminFilter())
async def cmd_sync_states(message: Message, db_session: AsyncSession):
    """اسکن کل ردیس و تطبیق آن با MySQL برای حذف زامبی‌های رسوب کرده"""
    await message.answer("⏳ <b>در حال اسکن و همگام‌سازی Redis و MySQL...</b>\nاین عملیات ممکن است چند ثانیه طول بکشد.", parse_mode="HTML")
    
    cursor = "0"
    zombies_killed = 0
    active_valid = 0
    
    # ── ۱. پاکسازی استیت‌های نامعتبر کاربران ──
    while cursor != 0:
        cursor, keys = await redis_client.scan(cursor=cursor, match="user:state:*", count=500)
        
        for key in keys:
            key_str = key.decode('utf-8') if isinstance(key, bytes) else key
            tg_id_str = key_str.split(":")[-1]
            
            if not tg_id_str.isdigit():
                continue
                
            tg_id = int(tg_id_str)
            state_data = await redis_client.hgetall(key)
            
            if not state_data:
                continue
                
            # تبدیل بایت به استرینگ
            state_data = {
            ...
            # اگر کاربر در وضعیت چت یا مچ باشد، با دیتابیس تطبیق داده می‌شود
            if status in ["matched", "chatting", "in_chat", "in_date"]:
                ...
                if not active_match:
                    is_zombie = True
                elif partner_id and str(active_match.user_one_id) != partner_id and str(active_match.user_two_id) != partner_id:
                    is_zombie = True
    ...
    # ── ۲. پاکسازی قفل‌های هرز (anti_skip_lock) ──
    cursor_lock = "0"
    locks_cleared = 0
    while cursor_lock != 0:
        cursor_lock, lock_keys = await redis_client.scan(cursor=cursor_lock, match="anti_skip_lock:*", count=500)
        
        for lk in lock_keys:
            lk_str = lk.decode('utf-8') if isinstance(lk, bytes) else lk
            match_id = lk_str.split(":")[-1]
            
            if match_id.isdigit():
                match_row = await db_session.get(MatchHistory, int(match_id))
                # اگر دیت تمام شده ولی قفل در ردیس گیر کرده است
                if not match_row or not match_row.is_active:
                    await redis_client.delete(lk)
                    locks_cleared += 1
                    
    await message.answer(
        f"✅ <b>عملیات پاک‌سازی و همگام‌سازی پایان یافت!</b>\n"
        f"━━━━━━━━━━━━━━━━━━━━\n"
        f"🧟‍♂️ زامبی‌های کشته شده (استیت منقضی): <b>{zombies_killed}</b>\n"
        f"🔐 قفل‌های هرز پاک شده: <b>{locks_cleared}</b>\n"
        f"🟢 استیت‌های فعال و معتبر: <b>{active_valid}</b>\n\n"
        f"<i>تمام ددلاک‌های رسوب کرده در سیستم از بین رفتند.</i>",
        parse_mode="HTML"
    )